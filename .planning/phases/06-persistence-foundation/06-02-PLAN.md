...
must_haves:
  truths:
    ...
    - "Branded types (Cents, PersonId, ItemId) are fully restored after page refresh — computeSplit produces identical results on rehydrated config"
  artifacts:
    ...
    - path: "src/store/billStore.ts"
      provides: "Modified bill store with persist middleware, currentSplitId, loadConfig, setCurrentSplitId, and merge-based deserialization"
  ...
  key_links:
    ...
    - from: "src/store/billStore.ts"
      to: "src/storage/deserializeBillConfig.ts"
      via: "import deserializeBillConfig for persist merge option to re-brand types on rehydration"
      pattern: "import.*deserializeBillConfig.*from.*storage/deserializeBillConfig"
---

<objective>
Create the history store and add persist middleware to the bill store, completing the localStorage persistence layer.

Purpose: This plan delivers the two core persistence capabilities: (1) the active bill auto-saves through page refresh via Zustand persist on useBillStore (PERS-01), and (2) completed bills can be explicitly saved to a new useHistoryStore that persists to its own localStorage key (PERS-02). Both stores ship with version: 1 and migrate stubs for future schema safety (PERS-03). Critically, the bill store's persist config wires deserializeBillConfig into the rehydration merge path so branded types (Cents, PersonId, ItemId) are fully restored after page refresh.
...
<task type="auto">
  <name>Task 2: Add persist middleware with deserializeBillConfig rehydration wiring and new actions to billStore</name>
  <files>
    src/store/billStore.ts
    src/store/billStore.test.ts
  </files>
...
Change the `useBillStore` export to wrap with persist middleware. CRITICAL: Include a `merge` function that calls `deserializeBillConfig` on the persisted config to re-apply branded type constructors (Cents, PersonId, ItemId) at the rehydration boundary. Without this, JSON.parse strips the branded types and the store receives unbranded values — violating 06-01's type-safety guarantee.
...
The `merge` function is the key wiring that resolves RESEARCH.md Open Question 1 and prevents Pitfall 1. When Zustand rehydrates from localStorage, it calls `JSON.parse` internally (via `createJSONStorage`), then passes the result to `merge`. By calling `deserializeBillConfig(p.config)` inside `merge`, we re-apply `cents()`, `personId()`, and `itemId()` constructors at the rehydration boundary — the ONLY place this matters for the bill store.
...
Run `npx tsc --noEmit` — no type errors.
  ...
  <done>
billStore.ts has persist middleware wrapping immer with partialize on config only. The persist `merge` function calls `deserializeBillConfig(persisted.config)` to re-apply branded type constructors at the rehydration boundary — branded types survive JSON round-trip through localStorage. New actions loadConfig, setCurrentSplitId, and updated reset are working. All existing tests pass unchanged. Three new tests verify the new functionality. Schema version is 1 with migrate stub. Full test suite passes.
  </done>
</task>
...
4. Verify in browser DevTools (Application > Local Storage):
   ...
   - Verify branded types survive refresh: open console, run `const s = useBillStore.getState(); console.log(typeof s.config.tip.amountCents)` — should be `number` (the branded type's runtime representation). Then run `computeSplit(s.config)` and confirm it returns the same result as before refresh.
...
- Branded types (Cents, PersonId, ItemId) are fully restored on rehydration via deserializeBillConfig in persist merge option
